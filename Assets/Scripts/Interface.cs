...
{
	//Initializes game objects and sprites
	...
	//Serves as the timer for the game
    void FixedUpdate()
    {
        mode.Update(this);
        Debug.Log("Update time: " + Time.deltaTime);
	...
	//Tels the game which camera and mode is currently active
	public void State(Screen n)
	{
		switch (n) {
			case Screen.Title : //Title screen
...
public class Title : screenMode
{
	...
	{
		if (Input.GetButton("Player1Fire") && Input.GetButton("Player2Fire")) //start buttons for the game
		{
...
public class Play : screenMode
{
	...
	public void OnGUI(Interface parent)
	{
		//Draws board at the beginning of every game
		playboard.Draw();
